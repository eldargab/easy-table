...
/**
 * Push the current row to the table and start the new one
 *
 * @returns {Table} `this`
 */
...
/**
 * Create a printer which will format the value with `printer`,
 * add the `prefix` to it and right align the whole thing
 *
 * @param {String} prefix
...
aggr.printer = function(prefix, printer) {
  return function(val, width) {
    var str = prefix + ' ' + printer(val)
